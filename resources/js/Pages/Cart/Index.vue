                    ...
                    <div v-if="cartItems.length > 0" class="lg:col-span-1">
                        <div class="bg-gray-50 p-6 rounded-lg">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="mt-6">
                                    <h3
                                        class="text-md font-medium text-gray-900 mb-2"
                                    >
                                        Dirección de envío
                                    </h3>
                                    <label
                    ...
                    <form @submit.prevent="submitAddress">
                        ...
                        <div class="mb-4">
                            ...
                            <input
                                ...
                                required
                            />
                            <p v-if="errors.city" class="text-red-600 text-sm">
                                {{ errors.city }}
                            </p>
                        </div>
                        <div class="mb-4">
                            ...
                            <input
                                ...
                                required
                            />
                            <p v-if="errors.state" class="text-red-600 text-sm">
                                {{ errors.state }}
                            </p>
                        </div>
                        <div class="mb-4">
...
onMounted(() => {
    console.log("Cart onMounted: initialItems", props.initialItems);

    // Limpiar el carrito primero para evitar duplicados
    cartStore.clearCart();

    // Sincronizar con los items del backend
    if (props.initialItems && props.initialItems.length > 0) {
        cartStore.syncWithBackend(props.initialItems);
    } else {
        // Si no hay items, limpiar localStorage también
...
const proceedToCheckout = async () => {
    ...
    try {
        await form.post(route("checkout.store"), {
            ...
            preserveScroll: true,
            onSuccess: () => {
                // Clear Pinia store and local storage
                cartStore.clearCart();
                toast.success("Pedido procesado correctamente", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 2000,
                });
                // No manual redirect needed; backend handles Stripe redirect
            },
            onError: (errors) => {
...
const removeItem = async (index) => {
    ...
    try {
        ...
        await form.post(route("cart.remove"), {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
