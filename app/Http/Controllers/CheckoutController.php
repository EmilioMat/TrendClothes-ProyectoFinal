...
class CheckoutController extends Controller
{
    public function store(Request $request)
    {
        ...
        Log::info('STRIPE_SECRET value', ['stripe_secret' => $stripeSecret]);
        ...
        foreach ($cartItems as $item) {
            if (!$item->product) {
                return redirect()->route('cart.index')->with('error', "El producto con ID {$item->product_id} ya no existe");
            }
        }
    ...
    public function success(Request $request, Order $order)
    {
        ...
        try {
            ...
            if ($session->payment_status === 'paid') {
                ...
                // Aquí se fuerza al frontend a limpiar el carrito
                return Inertia::render('Checkout/Success', [
                    'order' => $order,
                    'clearCart' => true, // propiedad adicional para el frontend
                ]);
            }
